...
function createPlayer(ac, source, options) {
  ...
  player.play = function (props) {
    ...
    if (!buffer) {
      console.warn('Buffer not found', props)
      return
    } else if (!connected) {
    ...
    const gain = velocity * player.volume // Note velocity scaled to player volume

    //console.log('PLAY NOTE', { note, gain })

    const node = createNode(note, buffer, { gain })
  ...
  player.connect = function (dest) {
    if (connected && this.dest) out.disconnect(this.dest)
    connected = true
    out.connect(dest)
    this.dest = dest
    return player
  ...
  player.getVolume = () => player.volume
...
function envelope (ac, options, opts) {
  ...
  envParams.forEach(function (name, i) {
