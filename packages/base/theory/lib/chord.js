let daccord = require('../utils/daccord')
...
function Chord(root, name) {
  ...
  if (bass) {
    let intervals = this.intervals, bassInterval, note
...
Chord.prototype = {
  ...
  quality: function() {
    let third, fifth, seventh, intervals = this.intervals
  ...
  chordType: function() { // In need of better name
    let length = this.intervals.length, interval, has, invert, i, name
  ...
  get: function(interval) {
    if (typeof interval === 'string' && interval in knowledge.stepNumber) {
      let intervals = this.intervals, i, length
