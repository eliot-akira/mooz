let scientific = require('../utils/scientificNotation')
let helmholtz = require('../utils/helmholtz')
let pitchFq = require('../utils/pitchFreq')
...
Note.prototype = {
  ...
  name: function() {
    let value = this.accidentalValue()
    let idx = this.coord[1] + knowledge.A4[1] - value * 7 + 1
  ...
  */
   ...
   */
  chroma: function() {
    let value = (vector.sum(vector.mul(this.coord, [12, 7])) - 3) % 12
  ...
  helmholtz: function() {
    ...
    let padchar = octave < 3 ? ',' : '\''
    let padcount = octave < 2 ? 2 - octave : octave - 3
  ...
  enharmonics: function(oneaccidental) {
    let key = this.key(), limit = oneaccidental ? 2 : 3

    return ['m3', 'm2', 'm-2', 'm-3']
      .map(this.interval.bind(this))
      .filter(function(note) {
        let acc = note.accidentalValue()
        let diff = key - (note.key() - acc)

        if (diff < limit && diff > -limit) {
          let product = vector.mul(knowledge.sharp, diff - acc)
  ...
  solfege: function(scale, showOctaves) {
    let interval = scale.tonic.interval(this), solfege, stroke, count
  ...
  scaleDegree: function(scale) {
    ...
    return scale.scale.reduce(function(index, current, i) {
      let coord = Interval.toCoord(current).coord
  ...
  durationInSeconds: function(bpm, beatUnit) {
    let secs = (60 / bpm) / (this.duration.value / 4) / (beatUnit / 4)
...
Note.fromString = function(name, dur) {
  let coord = scientific(name)
  if (!coord) coord = helmholtz(name)
  return new Note(coord, dur)
}

Note.fromKey = function(key) {
  let octave = Math.floor((key - 4) / 12)
  let distance = key - (octave * 12) - 4
  let name = knowledge.fifths[(2 * Math.round(distance / 2) + 1) % 7]
  let subDiff = vector.sub(knowledge.notes[name], knowledge.A4)
  let note = vector.add(subDiff, [octave + 1, 0])
  let diff = (key - 49) - vector.sum(vector.mul(note, [12, 7]))

  let arg = diff ? vector.add(note, vector.mul(knowledge.sharp, diff)) : note
  return new Note(arg)
}

Note.fromFrequency = function(fq, concertPitch) {
  let key, cents, originalFq
