let createParseError = ({ title, token, index }) => new Error({
  type: 'parse',
...
module.exports = function(symbol) {
  let c, parsing = 'quality', additionals = [], name, chordLength = 2
  let notes = ['P1', 'M3', 'P5', 'm7', 'M9', 'P11', 'M13']
  let explicitMajor = false

  function setChord(name) {
    let intervals = SYMBOLS[name]
  ...
  for (var i = 0, len = symbol.length; i < len; i++) {
    ...
    if (parsing === 'quality') {
      let sub3 = (i + 2) < len ? symbol.substr(i, 3).toLowerCase() : null
      let sub2 = (i + 1) < len ? symbol.substr(i, 2).toLowerCase() : null
      if (sub3 in SYMBOLS)
        name = sub3
      ...
      alterations.forEach(function(alt, altIndex, arr) {
        ...
        let next = arr[altIndex + 1], lower = alt.toLowerCase()
        if (alt === 'M' || lower === 'maj') {
          if (next === '7')
            ignore = true
