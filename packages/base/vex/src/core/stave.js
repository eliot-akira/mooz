...
export class Stave extends Element {
  ...
  resetLines() {
    ...
    for (let i = 0; i < 5; i++) { //this.options.num_lines
      const visible =
        this.options.num_lines===1
        ? (i===2)
        : this.options.num_lines===2
          ? (i===1 || i===3)
          : true
      this.options.line_config.push({ visible })
    }
    this.options.num_lines = 5
  ...
  format() {
    ...
    this.start_x = x
      + 10 // Left padding for start note
    ...
    this.end_x = (endModifiers.length === 1 ? this.x + this.width : x)
      - 15 // Right padding for last note
