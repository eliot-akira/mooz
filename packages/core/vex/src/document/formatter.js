...
module.exports = function(Vex) {
  ...
  const log = (...args) =>
    ...
    ? log('Vex.Flow.DocumentFormatter', args)
    : null
 ...
 */
  ...
  Vex.Flow.DocumentFormatter.prototype.getVexflowVoice =function(voice, staves) {
    ...
    const vfVoice = new Vex.Flow.Voice({ num_beats: voice.time.num_beats,
    ...
    const vexflowObjects = new Array()
  ...
  Vex.Flow.DocumentFormatter.prototype.getVexflowNote = function(note, options) {
    ...
    if (noteObj.clef && noteObj.clef==='tab') {

      //log('CREATE TAB NOTE', note, noteObj)

      noteObj.positions = note.positions || []

      const vfNote = new Vex.Flow.TabNote(noteObj)
    ...
    const vfNote = new Vex.Flow.StaveNote(noteObj)
    ...
    allVfVoices.forEach(function(v) {
      ...
      if (numTickables) {
        v.tickables.forEach((t) => {
          if (!t.text) return
          // Space for each letter in lyrics - TODO: Exact pixel widths?
  ...
  function(measure, vfStaves, context, options) {
    ...
    this.document.getStaveConnectors().forEach(function(connector) {
      ...
      if ((options.system_start && connector.system_start)
        ...
        if (connector.parts && connector.parts.length>1)  {

          const c = new Vex.Flow.StaveConnector(
            vfStaves[firstStave], vfStaves[lastStave]
          )

          //log('CONNECTOR START', connector, c)

          c.setType(type).setContext(context).draw()
