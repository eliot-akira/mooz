...
module.exports = function(Vex) {
  ...
  Vex.Flow.Backend.MusicXML.DEBUG = true
  ...
  Vex.Flow.Backend.MusicXML.prototype.getMeasure = function(m) {
    ...
    let measure = new Vex.Flow.Measure({ time: time })
    ...
    for (let i = 0; i < attrs.length; i++) {
      ...
      case 'key':
        ...
        if (fifths) {
          ...
          attrObject = this.fifthsToKey(parseInt(fifths.textContent), mode)
        }
        break
      case 'time': {

        if (attr.getElementsByTagName('senza-misura')[0]) {
          attrObject = { num_beats: 4, beat_value: 4, soft: true }
        } else {
        ...
        if (signTag) {
          ...
          if (number > 0) {
            ...
            if (measureNum in this.attributes
              ...
              attrObject = new Array(this.numStaves[partNum])
            }
      ...
      case 'notations':
        Array.prototype.forEach.call(elem.childNodes, function(notationElem) {
          ...
          case 'technical':
            ...
            console.log('NOTATION', notationElem)
