...
var _boundingboxcomputation = __webpack_require__(66);
...
var _canvascontext = __webpack_require__(67);

var _raphaelcontext = __webpack_require__(68);

var _svgcontext = __webpack_require__(69);
...
var _stavesection = __webpack_require__(70);

var _stavetempo = __webpack_require__(71);

var _stavetext = __webpack_require__(72);
...
var Vex = __webpack_require__(65);
var extendXML = __webpack_require__(80);
...
var _tabslide = __webpack_require__(73);

var _tuplet = __webpack_require__(18);

var _gracenote = __webpack_require__(52);

var _gracetabnote = __webpack_require__(74);

var _tuning = __webpack_require__(75);

var _keymanager = __webpack_require__(76);

var _stavehairpin = __webpack_require__(77);
...
var _tremolo = __webpack_require__(78);

var _stringnumber = __webpack_require__(29);

var _crescendo = __webpack_require__(79);
...
module.exports = function (Vex) {
  __webpack_require__(81)(Vex);
  __webpack_require__(82)(Vex);
  __webpack_require__(83)(Vex);
  __webpack_require__(84)(Vex);
  return Vex;
};
...
module.exports = function (Vex) {
  ...
  Vex.Flow.Measure = function (object) {
    if ((typeof object === "undefined" ? "undefined" : _typeof(object)) != "object") throw new Vex.RERR("ArgumentError", "Invalid argument to Vex.Flow.Measure");
    if (!object.time || !object.time.num_beats || !object.time.beat_value) throw new Vex.RERR("ArgumentError", "Measure must be initialized with nonzero num_beats and beat_value");
    this.time = Vex.Merge({}, object.time);

    this.attributes = {};
    if (_typeof(object.attributes) == "object") Vex.Merge(this.attributes, object.attributes);
    this.parts = new Array(1); // default to 1 part
    if (typeof object.getParts == "function") this.parts = object.getParts(); // Copy parts from first-class object
  ...
  Vex.Flow.Measure.prototype.setPart = function (partNum, part) {
    if (this.parts.length <= partNum) throw new Vex.RERR("ArgumentError", "Set number of parts before adding part");
  ...
  Vex.Flow.Measure.prototype.getStave = function (staveNum) {
    ...
    for (var i = 0; i < this.getNumberOfParts(); i++) {
      ...
      if (firstStaveForPart + part.getNumberOfStaves() > staveNum) return part.getStave(staveNum - firstStaveForPart);
  ...
  Vex.Flow.Measure.prototype.addNote = function (note) {
    if (this.getNumberOfParts() != 1) throw new Vex.RERR("ArgumentError", "Measure.addNote requires single part");
  ...
  Vex.Flow.Measure.Part = function (object) {
    if ((typeof object === "undefined" ? "undefined" : _typeof(object)) != "object") throw new Vex.RERR("ArgumentError", "Invalid argument to constructor");
    if (!object.time || !object.time.num_beats || !object.time.beat_value) throw new Vex.RERR("ArgumentError", "Constructor requires nonzero num_beats and beat_value");
    this.time = Vex.Merge({}, object.time);
    ...
    if (_typeof(object.options) == "object") Vex.Merge(this.options, object.options);

    if (typeof object.getVoices == "function") this.voices = object.getVoices();else if (object.voices instanceof Array) {
      ...
      this.voices = object.voices.map(function (voice) {
        ...
        return new Vex.Flow.Measure.Voice(Vex.Merge(Vex.Merge({}, voiceOptions), voice));
      });
    } else this.voices = new Array(1); // Default to single voice

    if (typeof object.getStaves == "function") this.staves = object.getStaves();else if (object.staves instanceof Array) {
      ...
      if (typeof object.staves == "number") this.staves = new Array(object.staves);else this.staves = new Array(1);
    }
  ...
  Vex.Flow.Measure.Part.prototype.getVoice = function (voiceNum) {
    if (!this.voices[voiceNum])
      // Create empty voice
      this.voices[voiceNum] = new Vex.Flow.Measure.Voice(Vex.Merge({ time: this.time }, this.options));
    return this.voices[voiceNum];
  };
  Vex.Flow.Measure.Part.prototype.setVoice = function (voiceNum, voice) {
    if (this.voices.length <= voiceNum) throw new Vex.RERR("ArgumentError", "Set number of voices before adding voice");
    this.voices[voiceNum] = new Vex.Flow.Measure.Voice(voice);
  };
  Vex.Flow.Measure.Part.prototype.getVoices = function () {
    for (var i = 0; i < this.getNumberOfVoices(); i++) {
      this.getVoice(i);
  ...
  Vex.Flow.Measure.Part.prototype.getStave = function (staveNum) {
    if (!this.staves[staveNum]) {
      // Create empty stave
      this.staves[staveNum] = new Vex.Flow.Measure.Stave(Vex.Merge({ time: this.time }, this.options));
    }
    return this.staves[staveNum];
  };
  Vex.Flow.Measure.Part.prototype.setStave = function (staveNum, stave) {
    if (this.staves.length <= staveNum) throw new Vex.RERR("ArgumentError", "Set number of staves before adding stave");
    this.staves[staveNum] = new Vex.Flow.Measure.Stave(Vex.Merge(Vex.Merge({}, this.options), stave));
  };
  Vex.Flow.Measure.Part.prototype.getStaves = function () {
    for (var i = 0; i < this.getNumberOfStaves(); i++) {
      this.getStave(i);
  ...
  Vex.Flow.Measure.Part.prototype.addNote = function (note) {
    if (this.getNumberOfVoices() != 1) throw new Vex.RERR("ArgumentError", "Measure.addNote requires single part");
  ...
  Vex.Flow.Measure.Voice = function (object) {
    ...
    if (object.notes instanceof Array) object.notes.forEach(function (note) {
      this.addNote(new Vex.Flow.Measure.Note(note));
    }, this);else this.notes = new Array();

    // Voice must currently be on a single stave
    if (typeof object.stave == "number") this.stave = object.stave;else this.stave = 0;
  ...
  Vex.Flow.Measure.Voice.prototype.addNote = function (note) {
    ...
    if (!note.rest && this.key && note.accidentals == null) {
      ...
      if (!this._accidentals) this._accidentals = Vex.Flow.Measure.Voice.keyAccidentals(this.key);
      ...
      noteObj.accidentals = noteObj.keys.map(function (key) {
        ...
        if (acc == "n") {
          ...
          if (accidentals[key] == acc) acc = null;else {
            accidentals[key] = acc;
            if (acc == null) acc = "n";
          }
        }
  ...
  Vex.Flow.Measure.Stave = function (object) {
    ...
    if (!object.time || !object.time.num_beats || !object.time.beat_value) throw new Vex.RERR("ArgumentError", "Constructor requires nonzero num_beats and beat_value");
    this.time = Vex.Merge({}, object.time);
    if (typeof object.clef != "string") throw new Vex.RERR("InvalidIRError", "Stave object requires clef property");
  ...
  Vex.Flow.Measure.Stave.prototype.addModifier = function (modifier) {
    ...
    if ((typeof modifier === "undefined" ? "undefined" : _typeof(modifier)) != "object" || typeof modifier.type != "string") throw new Vex.RERR("InvalidIRError", "Stave modifier requires type string property");
    // Copy modifier
    ...
    switch (modifier.type) {
      case "clef":
        if (typeof modifier.clef != "string") throw new Vex.RERR("InvalidIRError", "Clef modifier requires clef string");
        newModifier.clef = modifier.clef;
        break;
      case "key":
        if (typeof modifier.key != "string") throw new Vex.RERR("InvalidIRError", "Key modifier requires key string");
        newModifier.key = modifier.key;
        break;
      case "time":
        if (!modifier.num_beats || !modifier.beat_value) throw new Vex.RERR("InvalidIRError", "Time modifier requires nonzero num_beats and beat_value");
  ...
  Vex.Flow.Measure.Stave.prototype.deleteModifier = function (modifier) {
    if (typeof modifier != "string") throw new Vex.RERR("ArgumentError", "deleteModifier requires string argument");
  ...
  Vex.Flow.Measure.Stave.prototype.deleteAutomaticModifiers = function () {
    ...
    var newModifiers = new Array();
    this.modifiers.forEach(function (mod) {
      if (mod.automatic) anyDeleted = true;else newModifiers.push(mod);
    });
    this.modifiers = newModifiers;
    return anyDeleted;
  };
  ...
  Vex.Flow.Measure.Note = function (object) {
    ...
    if (object.keys instanceof Array)
      ...
      this.keys = object.keys.map(Vex.Flow.Measure.Note.Key);else this.keys = new Array();
    if (object.accidentals instanceof Array) {
      if (object.accidentals.length != this.keys.length) throw new Vex.RERR("InvalidIRError", "accidentals and keys must have same length");
    ...
    this.intrinsicTicks = object.intrinsicTicks > 0 ? object.intrinsicTicks : null;
    this.tickMultiplier = _typeof(object.tickMultiplier) == "object" && object.tickMultiplier ? new Vex.Flow.Fraction(object.tickMultiplier.numerator, object.tickMultiplier.denominator) : this.intrinsicTicks ? new Vex.Flow.Fraction(1, 1) : null;
    this.tuplet = _typeof(object.tuplet) == "object" && object.tuplet ? { num_notes: object.tuplet.num_notes,

      // NOTE: beats_occupied -> notes_occupied

      notes_occupied: object.tuplet.notes_occupied
    } : null;
    this.stem_direction = typeof object.stem_direction == "number" ? object.stem_direction : null;
    this.beam = typeof object.beam == "string" ? object.beam : null;
    this.tie = typeof object.tie == "string" ? object.tie : null;
    this.lyric = _typeof(object.lyric) == "object" && object.lyric ? { text: object.lyric.text } : null;
...
module.exports = function (Vex) {
  ...
  Vex.Flow.Backend.MusicXML.prototype.parse = function (data) {
    ...
    } else if (data instanceof Document) this.document = data;else {
      this.valid = false;
      throw new Vex.RERR("ArgumentError", "MusicXML requires XML string or DOM Document object");
    }
    this.documentElement = this.document.documentElement;
    if (this.documentElement.nodeName != 'score-partwise') throw new Vex.RERR("ArgumentError", "VexFlow only supports partwise scores");
    ...
    Array.prototype.forEach.call(this.documentElement.childNodes, function (node) {
      if (node.nodeName == "part-list") this.parsePartList(node);else if (node.nodeName == "part") {
        var measureNum = 0;
        for (var j = 0; j < node.childNodes.length; j++) {
          var measure = node.childNodes[j];
          if (measure.nodeName != "measure") continue;
          if (!(measureNum in this.measures)) this.measures[measureNum] = new Array();
            ...
            Array.prototype.forEach.call(elem.childNodes, function (groupElem) {
              switch (groupElem.nodeName) {
                case "group-symbol":
                  ...
                  break;
                case "group-barline":
                  if (groupElem.textContent == "yes") staveConnectors.push({ type: "single", measure_start: true,
                    system_end: true });
              }
            });
  ...
  Vex.Flow.Backend.MusicXML.prototype.isValid = function () {
    return this.valid;
  };
  ...
  Vex.Flow.Backend.MusicXML.prototype.getMeasure = function (m) {
    ...
    for (var p = 0; p < numParts; p++) {
      ...
      if (attrs.clef instanceof Array) for (var s = 0; s < this.numStaves[p]; s++) {
      ...
      for (var i = 0; i < noteElems.length; i++) {
        ...
        if (noteObj.chord) lastNote.keys.push(noteObj.keys[0]);else {
      ...
      for (var _i = 0; _i < part.getNumberOfVoices(); _i++) {
        if (_typeof(part.getVoice(_i)) == "object" && part.getVoice(_i).notes.length > 0) {
          newPart.setVoice(v, part.getVoice(_i));
          v++;
        }
  ...
  Vex.Flow.Backend.MusicXML.prototype.getStaveConnectors = function () {
    return this.staveConnectors;
  };

  Vex.Flow.Backend.MusicXML.prototype.parseAttributes = function (measureNum, partNum, attributes) {
    var attrs = attributes.childNodes;
    var attrObject = void 0;
    for (var i = 0; i < attrs.length; i++) {
      ...
      switch (attr.nodeName) {
        case "staves":
          ...
          break;
        case "key":
          attrObject = this.fifthsToKey(parseInt(attr.getElementsByTagName("fifths")[0].textContent));
          break;
        case "time":
          attrObject = attr.getElementsByTagName("senza-misura").length > 0 ? { num_beats: 4, beat_value: 4, soft: true } : {
        ...
        case "clef":
          ...
          var clef = sign == "G" && line == "2" ? "treble" : sign == "C" && line == "3" ? "alto" : sign == "C" && line == "4" ? "tenor" : sign == "F" && line == "4" ? "bass" : sign == "percussion" ? "percussion" : null;
          if (number > 0) {
            if (measureNum in this.attributes && partNum in this.attributes[measureNum] && this.attributes[measureNum][partNum].clef instanceof Array) attrObject = this.attributes[measureNum][partNum].clef;else attrObject = new Array(this.numStaves[partNum]);
  ...
  Vex.Flow.Backend.MusicXML.prototype.parseNote = function (noteElem, attrs) {
    ...
    Array.prototype.forEach.call(noteElem.childNodes, function (elem) {
      switch (elem.nodeName) {
        case "pitch":
          {
            var _step = elem.getElementsByTagName("step")[0].textContent;
            var _octave = parseInt(elem.getElementsByTagName("octave")[0].textContent);
            var alter = elem.getElementsByTagName("alter")[0];
            if (alter) switch (parseInt(alter.textContent)) {
              case 1:
                _step += "#";break;
              case 2:
                _step += "##";break;
            ...
            noteObj.keys = [_step + "/" + _octave.toString()];
          }break;
        case "type":
          ...
          break;
        case "dot":
          // Always follow type; noteObj.duration exists
          var duration = noteObj.duration,
              rest = duration.indexOf("r");
          if (noteObj.rest) duration = duration.substring(0, rest) + "dr";else duration += "d";
          noteObj.duration = duration;
          break;
        case "duration":
          var intrinsicTicks = new Vex.Flow.Fraction(Vex.Flow.RESOLUTION / 4 * parseInt(elem.textContent), attrs.divisions).simplify();
          if (isNaN(intrinsicTicks.numerator) || isNaN(intrinsicTicks.denominator)) throw new Vex.RERR("InvalidMusicXML", "Error parsing MusicXML duration");
        ...
        case "rest":
          ...
          break;
        case "grace":
          noteObj.grace = true;break;
        case "chord":
          noteObj.chord = true;break;
        case "voice":
        ...
        case "staff":
          ...
          break;
        case "stem":
          if (elem.textContent == "up") noteObj.stem_direction = 1;else if (elem.textContent == "down") noteObj.stem_direction = -1;
          break;
        case "beam":
        ...
        case "notations":
          Array.prototype.forEach.call(elem.childNodes, function (notationElem) {
            switch (notationElem.nodeName) {
              case "tied":
                // start-start/stop-stop vs begin-continue-end
                ...
                switch (tie) {
                  ...
                  case "stop":
                    ...
                    break;
                  default:
                    Vex.RERR("BadMusicXML", "Bad tie: " + tie.toString());
                }
                break;
              // TODO: tuplet
            }
    ...
    if (noteObj.rest && !noteObj.keys) {
      ...
      switch (clef) {
        case "bass":
          noteObj.keys = ["D/3"];break;
        case "tenor":
          noteObj.keys = ["A/3"];break;
        case "alto":
          noteObj.keys = ["C/4"];break;
        case "treble":default:
          noteObj.keys = ["B/4"];break;
      }
    }
    return noteObj;
  };
      ...
      for (var key in measureAttrs) {
        ...
        if (val instanceof Array) {
          ...
          for (var ind = 0; ind < val.length; ind++) {
            if (typeof attrs[key][ind] == "undefined" || typeof val[ind] != "undefined" && val[ind] != null) attrs[key][ind] = val[ind];
          }
        } else attrs[key] = val;
      }
    }
  ...
  Vex.Flow.Backend.MusicXML.prototype.fifthsToKey = function (fifths) {
    ...
    for (var i in Vex.Flow.keySignature.keySpecs) {
      ...
      if ((typeof spec === "undefined" ? "undefined" : _typeof(spec)) != "object" || !("acc" in spec) || !("num" in spec)) continue;
      if (fifths < 0 && spec.acc == "b" && spec.num == Math.abs(fifths) || fifths >= 0 && spec.acc != "b" && spec.num == fifths) return i;
    }
  };
  return Vex;
};
 ...
 * @author Daniel Ringwalt (ringw)
 */
module.exports = function extendXML(Vex) {
  ...
  Vex.Flow.Backend.IR.prototype.parse = function (object) {
    if (!Vex.Flow.Backend.IR.appearsValid(object)) throw new Vex.RERR("InvalidArgument", "IR object must be a valid document");

    // Force a first-class document object to get all measures
    if (typeof object.getNumberOfMeasures == "function" && typeof object.getMeasure == "function") {
   ...
   */
  Vex.Flow.Backend.IR.prototype.isValid = function () {
    return this.valid;
  };
  ...
  Vex.Flow.Backend.IR.appearsValid = function (object) {
    return (typeof object === "undefined" ? "undefined" : _typeof(object)) == "object" && object.type == "document";
  };
  ...
  Vex.Flow.Backend.IR.prototype.getStaveConnectors = function () {
    if (this.documentObject.staveConnectors) return this.documentObject.staveConnectors;
    if (typeof this.documentObject.getStaveConnectors == "function") return this.documentObject.getStaveConnectors();
    return [];
  };
  ...
  Vex.Flow.Document.prototype.init = function (data, options) {
    ...
    var backends = typeof this.options.backend == "function" ? [this.options.backend] : Vex.Flow.Document.backends;
  ...
  Vex.Flow.Document.prototype.getFormatter = function (formatterClass) {
    ...
    if (typeof formatterClass != "function") Formatter = Vex.Flow.DocumentFormatter.Liquid; // default class
  ...
  Vex.Flow.Document.prototype.getMeasureNumber = function (m) {
    return typeof this.backend.getMeasureNumber == "function" ? this.backend.getMeasureNumber(m) : m + 1;
  };
  ...
  Vex.Flow.Document.prototype.getMeasure = function (m) {
    ...
    if (typeof console != "undefined" && console.assert) console.assert(measure instanceof Vex.Flow.Measure, "Backend must return valid Vex.Flow.Measure");
  ...
  Vex.Flow.Document.prototype.getStaveConnectors = function () {
    if (_typeof(this.staveConnectors) != "object") {
      ...
      this.staveConnectors.forEach(function (connector) {
        if (connector.type == "single" && connector.parts[0] == 0 && connector.parts[connector.parts.length - 1] == lastPart && (connector.system_start || connector.measure_start)) haveSingleSystemStart = true;
      });
      if (!haveSingleSystemStart) this.staveConnectors.push({
        type: "single", system_start: true, parts: [0, lastPart] });

      // Add barlines to each part if necessary
      var partsHaveBarlines = [];
      this.staveConnectors.forEach(function (connector) {
        if (connector.type == "single" && connector.parts.length == 1 && connector.measure_start && connector.system_end) partsHaveBarlines[connector.parts[0]] = true;
      });
      for (var i = 0; i < numParts; i++) {
        if (!partsHaveBarlines[i]) this.staveConnectors.push({
...
module.exports = function (Vex) {
  ...
  Vex.Flow.DocumentFormatter.prototype.init = function (document) {
    if ((typeof document === "undefined" ? "undefined" : _typeof(document)) != "object") throw new Vex.RERR("ArgumentError", "new Vex.Flow.DocumentFormatter() requires Document object argument");
  ...
  Vex.Flow.DocumentFormatter.prototype.createVexflowStave = function (s, x, y, w) {
    ...
    s.modifiers.forEach(function (mod) {
      switch (mod.type) {
        case "clef":
          vfStave.addClef(mod.clef);break;
        case "key":
          vfStave.addKeySignature(mod.key);break;
        case "time":
          {
            var time_sig = void 0;
            if (typeof mod.time == "string") time_sig = mod.time;else time_sig = mod.num_beats.toString() + "/" + mod.beat_value.toString();
  ...
  Vex.Flow.DocumentFormatter.prototype.getStave = function (m, s) {
    if (m in this.vfStaves && s in this.vfStaves[m]) return this.vfStaves[m][s];
    if (typeof this.getStaveX != "function" || typeof this.getStaveWidth != "function") throw new Vex.RERR("MethodNotImplemented", "Document formatter must implement getStaveX, getStaveWidth");
    //console.log(m, this.document.getMeasure(m));
    var stave = this.document.getMeasure(m).getStave(s);

    if (!stave) return undefined;
    var vfStave = this.createVexflowStave(stave, this.getStaveX(m, s), this.getStaveY(m, s), this.getStaveWidth(m, s));
  ...
  Vex.Flow.DocumentFormatter.prototype.getVexflowVoice = function (voice, staves) {
    ...
    if (typeof voice.stave != "number") throw new Vex.RERR("InvalidIRError", "Voice should have stave property");
    ...
    for (var i = 0; i < voice.notes.length; i++) {
      ...
      if (note.beam == "begin") beamedNotes = [vfNote];else if (note.beam && beamedNotes) {
      ...
      if (note.tuplet) {
        if (tupletNotes) tupletNotes.push(vfNote);else {
          tupletNotes = [vfNote];
        ...
        if (tupletNotes.length == tupletOpts.num_notes) {
          ...
          tupletNotes.forEach(function (n) {
            vfVoice.addTickable(n);
          });
    ...
    if (typeof console != "undefined" && console.assert) console.assert(vfVoice.stave instanceof Vex.Flow.Stave, "VexFlow voice should have a stave");
  ...
  Vex.Flow.DocumentFormatter.prototype.getVexflowNote = function (note, options) {
    ...
    var i = 0;
    if (note.accidentals instanceof Array) note.accidentals.forEach(function (acc) {
  ...
  Vex.Flow.DocumentFormatter.prototype.getMinMeasureWidth = function (m) {
    if (!(m in this.minMeasureWidths)) {
      ...
      var context = Vex.Flow.Renderer.bolsterCanvasContext(canvas.getContext("2d"));
      ...
      this.minMeasureWidths[m] = Vex.Max(50, maxExtraWidth + noteWidth + maxTickables * 10 + 10);

      // Calculate minMeasureHeight by merging bounding boxes from each voice
      // and the bounding box from the stave
      var minHeights = [];
      // Initialize to zero
      for (var _i2 = 0; _i2 < vfStaves.length + 1; _i2++) {
        minHeights.push(0);
      ...
      allVfVoices.forEach(function (v) {
        if (v.stave !== lastStave) {
          if (i >= 0) {
            ...
            minHeights[i + 1] = lastBoundingBox.getH() + lastBoundingBox.getY();
          }
  ...
  Vex.Flow.DocumentFormatter.prototype.drawPart = function (part, vfStaves, context) {
    ...
    formatter.format(vfVoices, vfStaves[0].getNoteEndX() - vfStaves[0].getNoteStartX() - 10);
    var i = 0;
  ...
  Vex.Flow.DocumentFormatter.prototype.drawMeasure = function (measure, vfStaves, context, options) {
    var startStave = 0;
    ...
    this.document.getStaveConnectors().forEach(function (connector) {
      if (!(options.system_start && connector.system_start || options.system_end && connector.system_end || connector.measure_start)) return;
      var firstPart = connector.parts[0],
          lastPart = connector.parts[connector.parts.length - 1];
      var firstStave = void 0,
          lastStave = void 0;
      ...
      parts.forEach(function (part) {
        ...
        if (partNum == lastPart) lastStave = staveNum + part.getNumberOfStaves() - 1;
        staveNum += part.getNumberOfStaves();
        partNum++;
      });
      if (isNaN(firstStave) || isNaN(lastStave)) return;
      var type = connector.type == "single" ? Vex.Flow.StaveConnector.type.SINGLE : connector.type == "double" ? Vex.Flow.StaveConnector.type.DOUBLE : connector.type == "brace" ? Vex.Flow.StaveConnector.type.BRACE : connector.type == "bracket" ? Vex.Flow.StaveConnector.type.BRACKET : null;
      if (options.system_start && connector.system_start || connector.measure_start) {
        new Vex.Flow.StaveConnector(vfStaves[firstStave], vfStaves[lastStave]).setType(type).setContext(context).draw();
      }
      if (options.system_end && connector.system_end) {
        var stave1 = vfStaves[firstStave],
            stave2 = vfStaves[lastStave];
        var dummy1 = new Vex.Flow.Stave(stave1.x + stave1.width, stave1.y, 100);
        var dummy2 = new Vex.Flow.Stave(stave2.x + stave2.width, stave2.y, 100);
        new Vex.Flow.StaveConnector(dummy1, dummy2).setType(type).setContext(context).draw();
      }
    });
  };
  ...
  Vex.Flow.DocumentFormatter.prototype.drawBlock = function (b, context) {
    ...
    measures.forEach(function (m) {
      ...
      while (this.getStave(m, stave)) {
        stave++;
      }this.drawMeasure(this.document.getMeasure(m), this.vfStaves[m], context, { system_start: m == measures[0],
  ...
  Vex.Flow.DocumentFormatter.Liquid = function (document) {
    ...
    this.zoom = 0.8;
    this.scale = 1.0;
    if (typeof window.devicePixelRatio == "number" && window.devicePixelRatio > 1) this.scale = Math.floor(window.devicePixelRatio);
  };
  Vex.Flow.DocumentFormatter.Liquid.prototype = new Vex.Flow.DocumentFormatter();
  Vex.Flow.DocumentFormatter.Liquid.constructor = Vex.Flow.DocumentFormatter.Liquid;
  ...
  Vex.Flow.DocumentFormatter.Liquid.prototype.getBlock = function (b) {
    ...
    this.document.getMeasure(startMeasure).getStaves().forEach(function (s) {
      ...
      if (startMeasure == 0 && !s.getModifier("time")) {
        ...
        else if (_typeof(s.time) == "object") s.addModifier(Vex.Merge({ type: "time", automatic: true }, s.time));
      }
    });
    ...
    if (this.getMinMeasureWidth(startMeasure) + start_x + 10 >= this.width) {
      ...
      (function () {
        ...
        while (width < _this.width && curMeasure < numMeasures) {
          // Except for first measure, remove automatic modifiers
          // If there were any, invalidate the measure width
          if (curMeasure != startMeasure) _this.document.getMeasure(curMeasure).getStaves().forEach(function (s) {
            if (s.deleteAutomaticModifiers() && this.minMeasureWidths && curMeasure in this.minMeasureWidths) delete this.minMeasureWidths[curMeasure];
          }.bind(_this));
          width += _this.getMinMeasureWidth(curMeasure);
          curMeasure++;
        }
        ...
        for (var m = startMeasure; m <= endMeasure; m++) {
          measureRange.push(m);
        }_this.measuresInBlock[b] = measureRange;

        // Allocate width to measures
        var remainingWidth = _this.width - start_x - 10;
        for (var _m = startMeasure; _m <= endMeasure; _m++) {
          // Set each width to the minimum
          _this.measureWidth[_m] = Math.ceil(_this.getMinMeasureWidth(_m));
          remainingWidth -= _this.measureWidth[_m];
        }
        // Split rest of width evenly
        var extraWidth = Math.floor(remainingWidth / (endMeasure - startMeasure + 1));
        for (var _m2 = startMeasure; _m2 <= endMeasure; _m2++) {
          _this.measureWidth[_m2] += extraWidth;
        }remainingWidth -= extraWidth * (endMeasure - startMeasure + 1);
        _this.measureWidth[startMeasure] += remainingWidth; // Add remainder
        // Calculate x value for each measure
        _this.measureX[startMeasure] = start_x;
        for (var _m3 = startMeasure + 1; _m3 <= endMeasure; _m3++) {
          _this.measureX[_m3] = _this.measureX[_m3 - 1] + _this.measureWidth[_m3 - 1];
        }_this.blockDimensions[b] = [_this.width, 0];
      })();
    }
    ...
    for (var _i3 = startMeasure; _i3 <= endMeasure; _i3++) {
      var minHeights = this.getMinMeasureHeight(_i3);
  ...
  Vex.Flow.DocumentFormatter.Liquid.prototype.getStaveX = function (m, s) {
    if (!(m in this.measureX)) throw new Vex.RERR("FormattingError", "Creating stave for measure which does not belong to a block");
    return this.measureX[m];
  };

  Vex.Flow.DocumentFormatter.Liquid.prototype.getStaveWidth = function (m, s) {
    if (!(m in this.measureWidth)) throw new Vex.RERR("FormattingError", "Creating stave for measure which does not belong to a block");
  ...
  Vex.Flow.DocumentFormatter.Liquid.prototype.draw = function (elem, options) {
    ...
    while (this.getBlock(b)) {
      ...
      if (!this.canvases[b]) {
        ...
        for (var a = b + 1; this.getBlock(a); a++) {
          if (_typeof(this.canvases[a]) == "object") {
            elem.insertBefore(canvas, this.canvases[a]);
            break;
          }
        }if (!canvas.parentNode) elem.appendChild(canvas); // Insert at the end of elem
    ...
    while (_typeof(this.canvases[b]) == "object") {
...
//# sourceMappingURL=vex-0.0.1.js.map